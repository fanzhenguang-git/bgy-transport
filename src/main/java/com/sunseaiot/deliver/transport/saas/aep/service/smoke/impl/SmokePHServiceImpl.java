 ...
 */
@Slf4j
@Service("smokePHService")
public class SmokePHServiceImpl extends AepServiceImpl implements NotificationService, SmokePHService {
    ...
    @Value("${deliver.product-id.aep-633ph}")
    private String productId;

    @Value("${deliver.master-apikey.aep-633ph}")
    ...
    public void onAction(String deviceId, String actionName, Map<String, TypeValue> parameterMap) {
        try{
            ...
            if (parameterMap.size() < 0){
                log.error("SmokeServiceImpl ERROR:命令参数不全");
                return;
            }
            long valueData = 0;
            ...
            log.error("SmokeServiceImpl ERROR ", e);
        }
    }
    ...
    public void saveBusiness(PostDataChangeVo postDataChangeVo) {
        ...
        if (postDataChangeVo.getPayload().get("smoke_value") != null){
